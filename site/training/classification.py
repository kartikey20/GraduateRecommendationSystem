...
data['acceptance'] = data['acceptance'].map(
    lambda x: str(x).lower() in {'ad', 'offer'})
data['acceptance'] = data['acceptance'].map(
    lambda x: 1 if x in {'ad', 'offer'} else x)
data['acceptance'] = data['acceptance'].map(lambda x: 0 if x in {'rej'} else x)
data['top15'] = data['top'].map(lambda x: '15/211' in str(x))
data['top30'] = np.logical_or(
    data['top15'], data['top'].map(lambda x: '30/211' in str(x)))
...
for major in np.unique(data['major'].astype(str)):
    ...
    for university in np.unique(data['university']):
        ...
        dt = data[(data['university'] == university)
                  & (data['major'] == major)]
        # can define data size limit requirment here
        if dt.size > 18 & np.unique(dt['acceptance']).size > 1:
            print("%s: %s" % (major, university))
            ad = dt[dt['acceptance']]
            ad[['G-V', 'G-Q', 'G-A', 'gpa-norm', 'toefl', 'G-QV']] = ad[['G-V', 'G-Q', 'G-A', 'gpa-norm', 'toefl',
                                                                         'G-QV']].fillna(ad[['G-V', 'G-Q', 'G-A', 'gpa-norm', 'toefl', 'G-QV']].mean())  # replace na by mean
            rej = dt[~dt['acceptance']]
            rej[['G-V', 'G-Q', 'G-A', 'gpa-norm', 'toefl', 'G-QV']] = rej[['G-V', 'G-Q', 'G-A', 'gpa-norm',
                                                                           'toefl', 'G-QV']].fillna(rej[['G-V', 'G-Q', 'G-A', 'gpa-norm', 'toefl', 'G-QV']].mean())
            dt = pd.concat([ad, rej], ignore_index=False)
            dt.iloc[np.random.permutation(len(dt))]
            # feel free to tackle parameters
            classifier = svm.SVC(C=1.2, probability=True, class_weight='auto')
            classifier.fit(dt[['G-V', 'G-Q', 'G-A', 'gpa-norm',
                           'toefl',  'top15', 'top30', 'G-QV']], dt['acceptance'])
