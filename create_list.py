...
def create_list(gre_q, gre_v, gre_w, toefl, ranking, gpa, major, rank_preference, cost_preference):
    ...
    with open('trained_models', 'rb') as f:
    ...
    data = data.sort('sort-criterium', ascending=False)[['QSscore', 'qsrank', 'cost', 'sort-criterium', 'admission rate', 'chance']]

    #Output
    for i, (university, (QSscore, qsrank, cost, admission_rate, sort_criterium, predicted_admission_chance)) in enumerate(data.iterrows()):
        if i > 10:
            break
        print("%s: %f %f(%d) = %f - %f" % (university, cost, QSscore, qsrank, sort_criterium, predicted_admission_chance))
